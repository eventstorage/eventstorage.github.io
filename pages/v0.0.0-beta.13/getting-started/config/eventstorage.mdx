...
We can select our event store of choice. Azure Sql, Postgres and Sql Server are supported so far homefully
including EventStoreDb in the upcoming releases.
...
Now that we learned how to configure our event storage with basic settings, next we:
  - learn how to take our configuration further and see different projection modes supported
