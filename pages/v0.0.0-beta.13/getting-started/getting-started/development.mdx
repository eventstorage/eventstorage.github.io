...
Use `storage` configuration option to specify a connection string and a schema for your event storage.
```csharp title="Program.cs" showLineNumbers
builder.Services.AddEventStorage(storage =>
{
    storage.Schema = "es"; // default is es
    storage.ConnectionString = builder.Configuration["Connection"];
});
```

Select event storage of choice among Azure Sql, Postgres or Sql Server.
```csharp title="Program.cs" showLineNumbers
builder.Services.AddEventStorage(storage =>
{
    storage.Schema = "es"; // default is es
...
To create aggregate root and its corresponding domain logic, let's create another project as `Order` to hold
our domain by running the following in project root:
```sh
...
Restart the api and head over to Swagger again to run the newly created api to get order status.
</Steps.Step>
</Steps>
